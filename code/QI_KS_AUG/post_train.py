...
# dataset, dataloader
train_dataset = post_dataset.KoBARTQGPostQuestionInfillingDataset(train_path, tokenizer)
...
for epoch_id in range(epochs):
    for step_index, batch_data in tqdm(enumerate(train_dataloader), f"[TRAIN] EP:{epoch_id}", total=len(train_dataloader)):
            ...
            input_ids, input_ids_WhiteSpace, decoder_input_ids, labels = tuple(value.to(device) for value in batch_data.values())

            model_outputs_1 = model.forward(input_ids, decoder_input_ids, labels)
            model_outputs_2 = model.forward(input_ids_WhiteSpace, decoder_input_ids, labels)
