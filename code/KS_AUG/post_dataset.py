...
class KoBARTQGPostQuestionInfillingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if (len(content_id) + len(question_id) + 2) < self.max_len:
            input_ids = content_id + [self.tokenizer.sep_token_id] + question_WhiteSpace_id
        else:
            input_ids = content_id[:self.max_len - len(question_id) - 2] + [self.tokenizer.sep_token_id] + question_WhiteSpace_id
